...
from bop_toolkit.bop_toolkit_lib import visualization
...
p = {
  ...
  'result_filenames': [
    'path2your/GBOT_dataset/Liftpod/test/GBOTnormal_assembly-test.csv',
  ],

  # Folder containing the BOP datasets.
  #'datasets_path': config.datasets_path,
  'datasets_path': 'path2your/GBOT_dataset/Liftpod',

  # Folder for output visualisations.
  'vis_path': os.path.join('path2your/GBOT_dataset/Liftpod/eval_GBOT', 'vis_est_poses'),

  # Path templates for output images.
  'vis_rgb_path': os.path.join(
    '{vis_path}', '{result_name}', '{scene_id:s}', '{vis_name}.jpg'),
  'vis_depth_diff_path': os.path.join(
    '{vis_path}', '{result_name}', '{scene_id:s}',
...
for result_fname in p['result_filenames']:
  ...
  # Load object models.
  models = {}
  for obj_num, obj_id in enumerate(dp_model['obj_ids']):
    misc.log('Loading 3D model of object {}...'.format(obj_id))
    model_path = dp_model['model_dir'].format(obj_id=obj_id)
    model_color = None
    if not p['vis_orig_color']:
      model_color = tuple(colors[(obj_num) % len(colors)])
  ...
  for scene_id, scene_ests in ests_org.items():
    ...
    scene_gt = inout.load_scene_gt(
      dp_split['scene_gt_dir'].format(scene_id=scene_id))
      ...
      for ests_vis_id, ests_vis in enumerate(im_ests_vis):
        ...
        if p['vis_rgb']:
          if 'rgb' in dp_split['im_modalities']:
            rgb = inout.load_im(dp_split['rgb_dir'].format(
        ...
        depth = None
        if p['vis_depth_diff'] or (p['vis_rgb'] and p['vis_rgb_resolve_visib']):
          depth = inout.load_depth(dp_split['depth_dir'].format(
            scene_id=scene_id, im_id=im_id))
          depth *= scene_camera[im_id]['depth_scale']  # Convert to [mm].

        # Visualization name.
        if p['vis_per_obj_id']:
          vis_name = '{im_id:06d}_{obj_id:s}'.format(
        ...
        vis_rgb_path = None
        if p['vis_rgb']:
          vis_rgb_path = p['vis_rgb_path'].format(
        ...
        if p['vis_depth_diff']:
          vis_depth_diff_path = p['vis_depth_diff_path'].format(
            vis_path=p['vis_path'], result_name=result_name, scene_id=scene_id,
            vis_name=vis_name)
        print(vis_depth_diff_path)
        # Visualization.
