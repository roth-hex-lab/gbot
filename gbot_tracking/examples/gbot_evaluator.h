...
#ifndef M3T_EXAMPLES_GBOT_EVALUATOR_H_
#define M3T_EXAMPLES_GBOT_EVALUATOR_H_
...
/**
 * \brief Class that holds a tracker to evaluate it on the GBOT dataset.
...
class GBOTEvaluator {
 private:
  ...
  std::filesystem::path result_pose_path_{};
  static constexpr size_t kNCurveValues = 100;
  static constexpr m3t::Intrinsics kGBOTIntrinsics{607.249f, 607.165f, 639.165f,
                                                   364.761f, 1280,     720};
  const std::map<std::string, float> kObject2SizeMultiplier{
      {"NanoChuckbyPrima", 1.0f}};
 ...
 public:
  ...
  GBOTEvaluator(const std::string &name,
  ...
  void set_track_mode(TrackMode track_mode);
  ...
  float calcRotationAngle(Eigen::Matrix3f R1, Eigen::Matrix3f R2) const;
  float calcVectorDifference(Eigen::Vector3f v1, Eigen::Vector3f v2) const;
 ...
 private:
  ...
  bool SetUpTracker(
      ...
      std::shared_ptr<Assemblystate> *assembly_ptr) const;
  ...
  void YOLOdetector(
      const std::shared_ptr<m3t::Tracker> &tracker_ptr,
      std::vector<m3t::Transform3fA> &body2world_poses, std::vector<std::vector<cv::Point3f>> &kpts_gt) const;
  bool LoadPoses(
  ...
  static void SavePoses(const Result &result, const std::string &title,
  ...
  const std::vector<std::string> sequence_numbers_{};
  ...
  TrackMode track_mode_ = TrackMode::GBOT;
...
#endif  // M3T_EXAMPLES_GBOT_EVALUATOR_H_
