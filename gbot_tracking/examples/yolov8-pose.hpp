...
public:
    ...
    void calculate6dpose(const Object &obj,
                         std::vector<cv::Point3f> kpts_gt,
                         cv::Matx33d cam_K, m3t::Transform3fA &body2world_pose
    );
...
private:
    ...
    Loggers gLogger{nvinfer1::ILogger::Severity::kERROR};
...
void YOLOv8_pose::postprocess(
    ...
    for (int i = 0; i < num_anchors; i++) {
        ...
        auto kps_ptr = row_ptr + 4 + num_labels;
...
void YOLOv8_pose::calculate6dpose(const Object &obj,
                                  std::vector<cv::Point3f> kpts_gt,
                                  cv::Matx33d cam_K,
    ...
    for (int k = 0; k < num_point + 2; k++) {
        if (k < num_point) {
            ...
            kpts_2d.push_back(cv::Point2f(kps[k * 3], kps[k * 3 + 1]));
        }
    }
    ...
    //std::cout << "pose";
    bool solution;
    solution = cv::solvePnPRansac(kpts_gt, kpts_2d, cam_K, distCoeffs, rvec, tvec, false);
