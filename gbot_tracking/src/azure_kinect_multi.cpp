...
int AzureKinectMulti::RegisterID() {
...
bool AzureKinectMulti::UnregisterID(int id) {
  const std::lock_guard<std::mutex> lock{mutex_};
  return update_capture_ids_.erase(id);
}

bool AzureKinectMulti::SetUp(uint32_t device_index_) {
  const std::lock_guard<std::mutex> lock{mutex_};
  if (!initial_set_up_) {
    // Configure camera
...
bool AzureKinectMulti::UpdateCapture(int id, bool synchronized) {
  const std::lock_guard<std::mutex> lock{mutex_};
...
const k4a::capture &AzureKinectMulti::capture() const { return capture_; }

const k4a::calibration &AzureKinectMulti::calibration() const {
  return calibration_;
}

const Transform3fA *AzureKinectMulti::color2depth_pose() const {
  if (initial_set_up_)
...
const Transform3fA *AzureKinectMulti::depth2color_pose() const {
  if (initial_set_up_)
